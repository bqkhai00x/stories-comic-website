...
function onupdate(){
   ...
   type: "get",
   url: "getdata.php",
   data: "mss=14" +"&chapter=" + document.getElementById("chapter").value,
...
function comment(){
   ...
   type: "get",
   url: "setcomment.php",
   data: "mss=14"+"&name="+name+"&plot=" + textComment,
   success: function(msg){
...
<select id="chapter" class="selectchap">
    <?php include '../config.php'; 
                    $stmt=$conn->prepare("SELECT COUNT(category) AS t FROM Story,Category WHERE MSS = C_MSS AND C_MSS = 14");
                    $temp = 1;
                    $stmt->execute();
